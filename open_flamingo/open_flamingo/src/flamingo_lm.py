...
from .helpers import GatedCrossAttentionBlock, CrossMambaBlock
...
class FlamingoLayer_Mamba(nn.Module):
    """
    FlamingoLayer_Mamba is a wrapper around the GatedCrossAttentionBlock and DecoderLayer.
    """
...
class FlamingoLMMixin_Mamba(nn.Module):
    """
    Mixin to add cross-attention layers to a language model.
    """
    ...
    def init_flamingo_layers(self, gradient_checkpointing, residual=False):
        """
        Re initializes the FlamingoLayer_Mambas.
        Propagates any changes made to self.gated_corss_attn_layers or self.old_decoder_blocks
        """
    ...
    def forward(self, input_ids, attention_mask, **kwargs):
        ...
        # kwargs["attention_mask"] = attention_mask
        # import pdb;pdb.set_trace()
        return super().forward(input_ids=kwargs["input_ids"], output_hidden_states=kwargs['output_hidden_states'], use_cache=kwargs['use_cache'])  # Call the other parent's forward method
...
class FlamingoLayer_Mamba_CrossMambaBlock(nn.Module):
    """
    FlamingoLayer_Mamba is a wrapper around the GatedCrossAttentionBlock and DecoderLayer.
    """

    def __init__(
        self, cross_mamba_layer, decoder_layer, gradient_checkpointing=False, residual=False
    ):
        super().__init__()
        self.cross_mamba_layer = cross_mamba_layer
        self.decoder_layer = decoder_layer
        self.vis_x = None
        self.media_locations = None
        self.residual = residual

        if self.cross_mamba_layer is not None:
            self.cross_mamba_layer._use_gradient_checkpointing = (
                gradient_checkpointing
            )
        self.decoder_layer._use_gradient_checkpointing = gradient_checkpointing

    def clone_parameters(self):
        self.res_layer = copy.deepcopy(self.cross_mamba_layer)
        if self.res_layer is not None:
            self.res_layer.requires_grad_(False)
...
class FlamingoLMMixin_Mamba_CrossMambaBlock(nn.Module):
    """
    Mixin to add cross-attention layers to a language model.
    """
    ...
    def init_flamingo(
        ...
        self.cross_mamba_layers = nn.ModuleList(
            [
                CrossMambaBlock(
                    dim=lang_hidden_size, dim_visual=vis_hidden_size, config=self.config, layer_idx=layer_idx
                )
                if (layer_idx + 1) % cross_attn_every_n_layers == 0
                else None
                for layer_idx, _ in enumerate(self._get_decoder_layers())
    ...
    def init_flamingo_layers(self, gradient_checkpointing, residual=False):
        ...
        self._set_decoder_layers(
            nn.ModuleList(
                [
                    FlamingoLayer_Mamba_CrossMambaBlock(
                        cross_mamba_layer, decoder_layer, gradient_checkpointing, residual=residual
                    )
                    for cross_mamba_layer, decoder_layer in zip(
                        self.cross_mamba_layers, self.old_decoder_blocks
                    )
                ]
            )
        )
    ...
    def forward(self, input_ids, attention_mask, **kwargs):
        ...
        # kwargs["attention_mask"] = attention_mask
        # import pdb;pdb.set_trace()
        # return super().forward(input_ids=kwargs["input_ids"])  # Call the other parent's forward method
        return super().forward(input_ids=kwargs["input_ids"], output_hidden_states=kwargs['output_hidden_states'], use_cache=kwargs['use_cache'])  # Call the other parent's forward method
